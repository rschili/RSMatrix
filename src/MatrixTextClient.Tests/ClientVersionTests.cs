...
public class ClientVersionTests
{
    ...
    {
        var version = new ClientVersion(1, 2, 3, "metadata");
    ...
    {
        var version = new ClientVersion(1, 2, null, "metadata");
    ...
    {
        await Assert.That(ClientVersion.TryParse(null!, out var version)).IsFalse();
        await Assert.That(ClientVersion.TryParse(string.Empty, out version)).IsFalse();
        await Assert.That(ClientVersion.TryParse("   ", out version)).IsFalse();
    }

    [Test]
    public async Task TryParse_ShouldReturnFalse_WhenInputIsInvalid()
    {
        await Assert.That(ClientVersion.TryParse("invalid", out var version)).IsFalse();
        await Assert.That(ClientVersion.TryParse("v1.2.3", out version)).IsFalse();
        await Assert.That(ClientVersion.TryParse("r1.2-invalid", out version)).IsFalse();
    }

    [Test]
    public async Task TryParse_ShouldReturnTrue_WhenInputIsValid()
    {
        await Assert.That(ClientVersion.TryParse("v1.2", out var version)).IsTrue();
        ...
        await Assert.That(ClientVersion.TryParse("r1.2.3", out version)).IsTrue();
        await Assert.That(version).IsNotNull();
        await Assert.That(version!.X).IsEqualTo(1);
        await Assert.That(version.Y).IsEqualTo(2);
        await Assert.That(version.Z).IsEqualTo(3);
        ...
        await Assert.That(ClientVersion.TryParse("v1.2-metadata", out version)).IsTrue();
        await Assert.That(version).IsNotNull();
        await Assert.That(version!.X).IsEqualTo(1);
        await Assert.That(version.Y).IsEqualTo(2);
        await Assert.That(version.Z).IsNull();
    ...
    {
        var version1 = new ClientVersion(1, 2, 3, null);
        var version2 = new ClientVersion(1, 2, 3, null);
        var version3 = new ClientVersion(1, 2, 4, null);
        var version4 = new ClientVersion(1, 3, null, null);
    ...
    {
        var version1 = new ClientVersion(1, 2, 3, "metadata");
        var version2 = new ClientVersion(1, 2, 3, "metadata");
        var version3 = new ClientVersion(1, 2, 4, "metadata");
        var version4 = new ClientVersion(1, 2, 3, "other");
    ...
    public async Task SortListOfStringsByVersionComparer()
    {
        ...
        var parsedVersions = versions.Select(v => ClientVersion.TryParse(v, out var version) ? version : null).ToList();
        var sortedVersions = parsedVersions.OrderBy(v => v, ClientVersion.Comparer.Instance).Select(v => v!.VersionString).ToList();
